...
public class Lighting : MonoBehaviour
{
    ...
    public Color ambientLight = new Color(0.1f, 0.1f, 0.1f, 1);
    ...
    public async void BakeLighting() {
        ...
        lightmapStep = new Vector2(su.TAU / (float)resolution[0], Mathf.PI / (float)resolution[1]);
            ...
            pixelColors = await Task.Run(() => {
                for (int x = 0; x < resolution[0]; x++)
                {
                    for (int y = 0; y < resolution[1]; y++)
                    {
                        ...
                        Vector2 sphPos = start + new Vector2(lightmapStep.x * (0.5f + x), lightmapStep.y * (0.5f + y)); // spherical position of pixel being calculated
        ...
        if (!stopBaking) {
            AssetDatabase.CreateAsset(textures, "Assets/Textures/Lightmap/lightmap.asset");
            lightmaps = textures;
    ...
    public bool shadowRay(Vector3 pos, Vector3 lpos, int layer, List<SphCircle> circles, List<SphGon> gons, List<SphShape> shapes) {
        ...
        for (int j = 0; j < gons.Count; j++)
        {
            ...
            {
                if (!collided && (gons[j].layer > layer)){
        ...
        for (int j = 0; j < shapes.Count; j++)
        {
            ...
            {
                if (!collided && (shapes[j].layer > layer)){
    ...
    public void SaveLightmapsAsPNG(Texture2DArray _textures)
    {
        for (int i = 0; i < _textures.depth; i++)
        {
            ...
            var dirPath = Application.dataPath + "/../Assets/Textures/Lightmap/LightmapPreview" + i + ".png";
